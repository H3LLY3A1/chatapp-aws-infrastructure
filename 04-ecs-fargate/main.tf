...
# Dedicated subnets for RDS (no route to internet – only VPC-local traffic)
...
resource "aws_s3_bucket_public_access_block" "images" {
  ...
  restrict_public_buckets = true // dodatkowa ochrona przed przypadkowym udostępnieniem bucketu
}
...
resource "aws_s3_bucket_versioning" "images" {
  ...
  versioning_configuration {
    status = "Suspended" // wersjonowanie jest opcjonalne, można je włączyć zmieniając na "Enabled"
  }
}
...
resource "aws_s3_bucket_cors_configuration" "images" {
  ...
  cors_rule {
    ...
    expose_headers  = ["ETag"] //aws s3 zwraca ETag w odpowiedzi na PUT, przydatne do zarządzania wersjami plików
